...
const getTargetData = async()=>{  
  await axios.get(import.meta.env.VITE_APP_URL + "/target/all")
  .then((res)=>{
    if(res?.status!=200){
...
const getStatusForTargets = async()=>{
  await axios.get(import.meta.env.VITE_APP_URL + "/status/all")
  .then((res)=>{
    if(res?.status!=200){
...
const getStatusForTarget = async(id)=>{
  await axios.get(import.meta.env.VITE_APP_URL + `/status/all/${id}`)
  .then((res)=>{
    if(res?.status!=200){
      console.error("Failed to Status Data")
      return
